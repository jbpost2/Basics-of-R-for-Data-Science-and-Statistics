...
> - How to visualize table for two?  
    <ul>
      ...
      <li> Side-by-side bar plot (Require work!)</li>
    </ul>  
...
g<-ggplot(data=twoWayData %>% drop_na(embarked), aes(x=as.factor(embarked),y=count))+
  geom_bar(stat="identity",position="dodge",aes(fill=as.factor(survived))) + 
    labs(x="City Embarked", title="Bar Plot of Embarked City for Titanic Passengers") + 
  scale_x_discrete(labels=c("Cherbourg","Queenstown","Southampton")) + 
  scale_fill_discrete(name="Surived",labels=c("No","Yes"))
...
- Color dots:  
    ...
    + If consistent or defined elsewhere, can go outside aes
...
```{r,eval=TRUE,echo=FALSE,message=FALSE}
g<-ggplot(CO2,aes(x=uptake))+geom_histogram()
g
```
...
```{r,eval=TRUE,echo=FALSE,message=FALSE}
g<-ggplot(CO2,aes(x=uptake))+
  geom_histogram(color="blue",fill="red",linetype="dashed",size=2,binwidth=3)
g
```
...
correlation<-cor(scoresFull$homeRushYds,scoresFull$HFinal)
g<-ggplot(scoresFull,aes(x=homeRushYds,y=HFinal))+
  geom_point()+geom_smooth()+geom_smooth(method=lm,col="Red") + geom_text(x=315,y=10,size=5,label=paste0("Correlation = ",round(correlation,2)))
g
```
...
```{r,eval=TRUE,echo=FALSE,message=FALSE}
g<-ggplot(scoresFull,aes(x=homeRushYds,y=HFinal))+
  geom_point()+geom_smooth()+geom_smooth(method=lm,col="Red") + geom_rug()
g
```
...
## Activity 
- [**Basic Analysis Activity** instructions](http://www4.stat.ncsu.edu/~post/DataScienceR/A nalysisActivity.html) available on web  
