...
- Useful for indexing a data set
```{r}
iris<-tbl_df(iris)
iris
```
...
- Useful for indexing a vector
- Better way: `filter()` from `dplyr` (installed with `tidyverse`)
...
- Create new variable for **Large Setosa** flowers
```{r,error=TRUE}
ifelse((iris$Petal.Length>1.5)&(iris$Petal.Width>0.3)&(iris$Species=="setosa"),
       "L-S","NotL-S")
```
...
```{r}
transmute(iris,Size=
          ifelse((Petal.Length>1.5)&(Petal.Width>0.3)&(Species=="setosa"),
                 "L-S","NotL-s"))
```
...
- "Starter" file there if you'd like  
